...
async def test_authenticate_user_success(fake_users):
    ...
    assert user.link_ids == fake_users.safe_resp.link_ids
    assert user.folder_ids == fake_users.safe_resp.folder_ids
...
async def test_get_current_user_from_token_success(fake_users):
    ...
    with patch(
        "src.auth.infrastructure.auth.authenticator.UserRepositoryManager",
        lambda *_: FakeUserRepository(),
    ):
        result = await get_current_user_from_token(
            access_token=token, session=None, client=None
        )
