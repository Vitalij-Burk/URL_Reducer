...
def test_deserialize_user_from_cache_success(fake_users):
    ...
    assert str(deserialized_user.user_id) == fake_loaded_cache["user_id"]
    assert deserialized_user.hashed_password == fake_loaded_cache["hashed_password"]
    assert deserialized_user.link_ids == fake_loaded_cache["link_ids"]
    assert deserialized_user.folder_ids == fake_loaded_cache["folder_ids"]
